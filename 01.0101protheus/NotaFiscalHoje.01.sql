SELECT
  ...
  , COALESCE( SF2.F2_TXMOEDA,0)
      AS TAXA_MOEDA
  , CASE 
      WHEN SD2.D2_FILIAL = SLX.LX_FILIAL 
      AND SD2.D2_DOC = SLX.LX_CUPOM 
      AND SD2.D2_SERIE = SLX.LX_SERIE
      THEN 
        CASE 
        WHEN SLX.LX_TPCANC = 'D' 
        AND SLX.LX_PRODUTO = SD2.D2_COD 
        THEN 'DEVOLVIDO'
        WHEN SLX.LX_TPCANC = 'A' 
        THEN 'CANCELADO'
        ELSE 'N'
        END
      ELSE 'N'
...
FROM SD2010 SD2
  ...
  LEFT JOIN SAH010 SAH
    ...
    AND SAH.D_E_L_E_T_ = ' '
  LEFT JOIN  SLX010 SLX 
    ON SLX.LX_FILIAL = SD2.D2_FILIAL 
    AND SLX.LX_CUPOM = SD2.D2_DOC
    AND SLX.LX_SERIE = SD2.D2_SERIE
    AND SLX.D_E_L_E_T_ = ' '
...
WHERE
  F2_EMISSAO BETWEEN CONVERT(varchar(8), DATEADD(month, -1, GETDATE()), 112) AND CONVERT(varchar(8), GETDATE(), 112)
